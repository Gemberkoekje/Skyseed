...
> **Status:** the audit + triage are done, the scope decision is **FULL SCOPE** (see the **Scope decision**
> section below), and the highest-value item — the **BWG villages** — is **fully shipped and signed off in-game**
> (all 6 styles × 3 tiers + the door-flush #72 + the cypress bayou #73 + the #14 biome-reachability check, v0.176.0–
> v0.188.0; its plan was retired into the changelogs). **Open here:** the **aspen manor (#26)** and **bog trial
> (#27)** resurrections with their vertical-jigsaw mitigation (#28) and on-pad assembly checks (#29), the per-step
> release-hygiene rider (#30), optional prairie houses / rugged fossil (#49), the deferred dedicated-seeds
> alternative (#60), and the recorded-only net-new-structures long tail (#68). Priorities in
> [`../PLANOFPLANS.md`](../PLANOFPLANS.md).
>
> **Reusable structure engine (inherited by the manor #26 / bog trial #27):** the hermetic **string-id `.nbt`
> engine** the villages shipped — mod blocks authored as vanilla analogs, real ids substituted at write time
> (`Mat` + `StructureWriter`'s `modNames` side-map) so no mod is ever on the classpath and CI stays clean — plus
> the **2-build regen dance** + stale-`.nbt` staging trap ([[skyseed-structure-staging]]) when pieces regenerate.
...
|---|---|---|---|
| **village** — 6 styles | `biomeswevegone:village/<style>/…` | `village_center` / `hamlet` | ✅ shipped + signed off (v0.176.0–v0.188.0, see CHANGELOG) |
...
2. **Dedicated seeds (#60):** only if a structure should be a *goal* in itself (cf. the dedicated-seed bar in the
   PLANOFPLANS Decisions log). Judged likely
   unnecessary — the mansion / trial seeds already exist. Kept as a recorded fallback.
3. **Author our OWN jigsaw set in themed palettes** — ⭐ the route the **villages actually took** (user's call:
   more control + variety; the hermetic string-id `.nbt` engine described in the status banner above). **The
   manor/bog-trial vehicle choice (route 1 vs 3) is the first decision of #26/#27** — route 3 avoids the
   foreign-pool risk at the cost of authoring the buildings.
...
1. ✅ Prerequisite (the BWG wood-biome adaptation) — shipped (see CHANGELOG).
2. ✅ **Villages** — shipped + signed off in-game (see CHANGELOG; plan retired).
