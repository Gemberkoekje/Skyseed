...
| Theme overrides | The **`skyseed:theme_override`** datapack registry — a merge layer that appends/prepends `biome_overrides` (and jigsaw bands) onto base themes without editing them, powering the **first-party inert-without-the-mod compat**: Create (zinc), Mystical Agriculture (ores on Ancient/Lush/Nether-Soul), Oh The Biomes We've Gone, and Farmer's Delight (wild crops on the biome islands, wild rice in Lush/Aquatic ponds, End's/My Nether's delights on the Chorus/Nether seeds) |
...
Skyseed itself optionally integrates with Modonomicon and Patchouli (guide backends) and ships first-party, inert-without-the-mod compat content for Oh The Biomes We've Gone, Mystical Agriculture, Create, and Farmer's Delight — so it drops cleanly into either pack (or a vanilla install).
