...
| Theme overrides | The **`skyseed:theme_override`** datapack registry — a merge layer that appends/prepends `biome_overrides` (and jigsaw bands) onto base themes without editing them, powering the **first-party inert-without-the-mod compat**: Create (zinc), Immersive Engineering (metal ores on the mining islands), Applied Energistics 2 (certus quartz + the Meteorite island), Mystical Agriculture (ores on Ancient/Lush/Nether-Soul), Quark (stone/geode families on the mining islands), Oh The Biomes We've Gone, and Farmer's Delight + its End's/My Nether's add-ons (wild crops on the biome islands, wild rice in Lush/Aquatic ponds, delights on the Chorus/Nether seeds) |
...
- **Remaining vanilla blocks** — essentially complete; the last notable one, the **copper bulb**, landed with the Trial Chamber vanilla redesign (it lights the chambers). The Nether- and End-gated block sets landed with their chapters.
...
- **"Grow your own world"** (`Modpack-growyourownworld/`) — the full pack: a ~125-mod manifest (`mods.txt`) built around **Create** (+ addons + the Crafts & Additions FE bridge + Flux Networks), **Oh The Biomes We've Gone**, **Mystical Agriculture / Botany Pots**, **Quark**, the **Farmer's Delight** family (base + End's / My Nether's / Ocean's / Autochef's / Chef's / Chopper's / FD Extended), **Immersive Engineering** (+ Petroleum + flight — the tech backbone), **Applied Energistics 2** (+ the meteorite island), Silent Gear, Sophisticated storage, a nine-chapter FTB quest spine, and a full shader/visual stack — each content mod wired into island generation via `theme_override` so its blocks have somewhere to grow. The whole content-mod wave (IE, AE2, Farmer's Delight, Quark) has shipped; remaining work is tracked in [PLANOFPLANS.md](PLANOFPLANS.md) and its plan docs (`Modpack-growyourownworld/*PLAN.md`).

Skyseed itself optionally integrates with Modonomicon and Patchouli (guide backends) and ships first-party, inert-without-the-mod compat content for Oh The Biomes We've Gone, Mystical Agriculture, Create, Immersive Engineering, Applied Energistics 2, Quark, and the Farmer's Delight family — so it drops cleanly into either pack (or a vanilla install).
